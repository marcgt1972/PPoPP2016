...
network training. Neural networks require a training stage which has typically
...
linear algebra subroutines (BLAS). All the widely used DNN frameworks (e,g.: Caffe \cite{Caffe}, Theano \cite{Theano}
or Torch \cite{Torch}) follow this approach. When it comes to optimization, 
GPU acceleration \cite{NVIDIA} has been the most spread widely adopted strategy and they have
...
libraries like cuBLAS  \cite{cuBLAS} and cuDNN \cite{cuDNN}. Unfortunately, this approach lacks from generality.
...
only DNN transformations that are very well understood and no 
...
datasets, MNIST \cite{MNIST} and CIFAR-10 \cite{CIFAR10}. In particular, in a \todo{16-core Xeon XXXX 
